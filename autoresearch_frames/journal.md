...
### Run 31 - 2026-04-24 - OOF-optimal blend weight grid search on 468-feature set
- Hypothesis: the 0.75/0.25 blend weight was hand-picked in Run 21. Grid-search w ∈ {0, 0.02, ..., 1.0} on OOF AUC (shared foldid between ENET via `keep=TRUE` and XGB via `folds=foldid_list, prediction=TRUE`) to see if OOF picks a different optimum.
- Config: compute OOF ENET probs from `fit.preval[, lambda.min]`, OOF XGB from `xgb_cv$pred`. Grid OOF AUC over w, pick best, apply to test predictions.
- Result: AUC **0.81695** | ACC **0.75000** | Baseline ACC **0.72640** | N_FEATURES **468** | DUR **27.71s**
- OOF picks **w=1.00** (ENET-only) with OOF AUC 0.81060 vs OOF AUC at w=0.75 = 0.81013. On test, however, w=0.75 gives **0.81771** and w=1.00 gives **0.81695** (direct from Run 22 diagnostic + confirmed here). OOF and test disagree on direction.
- Decision: **DISCARD**. AUC regressed `-0.00076` vs Run 21. Classical OOF-overfit-to-CV-noise case: the blend weight is picked on the CV fold distribution, which doesn't match the test fold's noise. Hand-picked 0.75 captures real XGB diversity that OOF CV underestimates.
- Signal for next run: OOF stacking is unreliable at this scale (5424 train rows across 5 folds = ~1085 per fold; the per-fold AUC noise dominates a 0.00005-level w-grid signal). After 10+ consecutive DISCARDs from the Run 21 anchor, the iteration has exhausted the obvious levers. Two genuinely untried families: (a) a **second ENET arm trained on log-transformed gold features** for multiplicative signal; (b) **GAM via `mgcv` on top-10 features** for a smooth-spline alternative to polynomials. Try (a) first - it's cheaper and aligns with the existing ENET pipeline.

### Run 30 - 2026-04-24 - ENET-residual XGB boosting (replace blend with correction)
- Hypothesis: blending ENET+XGB has two models trained on the same target; XGB finds patterns similar to ENET rather than complementary ones. Re-target XGB to model **ENET OOF residuals** directly - predict `y - oof_enet_prob` as a regression problem. Then final prediction = ENET + XGB correction. This forces XGB to focus on what ENET misses.
- Config: enable `keep=TRUE` in `cv.glmnet` to get OOF predictions. Compute residuals `r = y_train - oof_enet_prob`. Train XGB with `objective="reg:squarederror"` on `(X_train, r)`. Test: `pmin(pmax(enet_prob + xgb_residual, 0.001), 0.999)`.
- Result: AUC **0.81117** | ACC **0.74336** | Baseline ACC **0.72640** | N_FEATURES **468** | DUR **33.94s**
- Decision: **DISCARD**. Catastrophic `-0.00654` AUC regression. OOF residuals have `sd=0.42` (near the max 0.5 for 0/1 outcomes) - meaning ENET's residuals look near-random and don't carry a structured signal XGB can model. XGB fits small patterns (pred sd=0.038) that are mostly noise, and adding them to ENET corrupts the otherwise-well-ranked scores.
- Signal for next run: classical blending beats residual boosting here because ENET's errors aren't structurally learnable from the same features. Test AUC 0.81771 is within rounding of the CV estimate 0.81090 plus noise - the model is at or near the information ceiling of this representation. Next conservative run: revisit **OOF-optimal blend weight** (Run 6 did this on 442-feature Run-5 set; with 468 features and polynomials the OOF optimum may differ from the hand-picked 0.75). This is a small tuning run after three big structural swings.

### Run 29 - 2026-04-24 - free polynomial features from ENET penalty (penalty.factor=0)
- Hypothesis: the cvAUC curve is flat across alpha because the polynomial features get shrunk by both L1 (dropped entirely) and L2 (pulled to zero). `penalty.factor=0` on the 8 Run-18 polynomials keeps them always in the model at their OLS magnitude while other 460 features keep full penalty.
- Config: add `pen_factor` vector, `0` for poly feature indices, `1` elsewhere, pass to `cv.glmnet(penalty.factor=...)`. Everything else unchanged from Run 21.
- Result: AUC **0.81329** | ACC **0.74853** | Baseline ACC **0.72640** | N_FEATURES **468** | DUR **26.02s**
- Decision: **DISCARD**. Biggest regression in the loop: `-0.00442` AUC. ENET cvAUC dropped to 0.80929 (from 0.81090); best alpha flipped to 1.0 (pure L1) because L1 on the non-polynomial features tried to compensate for the now-unbounded poly magnitudes. Unpenalized polynomials grow to noise-tracking magnitudes and dominate the linear combination on test.
- Signal for next run: ENET's existing shrinkage on the polynomial features is load-bearing, not a bottleneck. The polynomials are useful *because* they're regularized. Don't touch penalty.factor on high-leverage features. Next direction: **let ENET see raw × polynomial interactions for top features** - specifically `f_15_gold_diff * f_5_gold_diff` (captures "lead that grew vs lead that shrunk") and `f_15_gold_diff * gold_adv_time_share` (captures "lead with persistence"). These are 2nd-order cross-terms that the diagonal polys can't express.

### Run 28 - 2026-04-24 - pressure / threat-proximity features (5 new)
- Hypothesis: gold leads pay off non-linearly near "snowball" thresholds (~2500 gold) where map pressure forces objective trades. Encode threshold-quadratics for both snowball (lead > 2500) and deadzone (deficit < -2500); add tower-pushing acceleration (tower_accel_5_15) and late-phase tower push (tower_late_push); align structural diffs with gold sign (aligned_threat_15).
- Config: append 5 threshold / structural features in `eng()`. Everything else unchanged from Run 21.
- Result: AUC **0.81679** | ACC **0.74926** | Baseline ACC **0.72640** | N_FEATURES **473** | DUR **28.13s**
- Decision: **DISCARD**. AUC regressed `-0.00092` vs Run 21. ENET cvAUC identical to Run 21 (0.81090) - the new features got regularized away; XGB cvAUC rose `+0.00072` but the test AUC dropped. The snowball-quadratic and deadzone-quadratic overlap closely with the already-present `gold15_sq`/`gold15_cu`; the tower accel/late-push features overlap with `gold_accel_5_15`/`tempo_delta_10_15`.
- Signal for next run: threshold-quadratics don't add on top of Run 18's diagonal polynomials because they live in the same function class. Next: move away from gold/structural features entirely. **Early-game micro-advantage features** - things that don't show up in minute-15 aggregates but influence win rate: first blood timing bucket, dragon-soul pacing indicators (`f_15_dragons >= 3`), solo-lane kill-participation proxies, side-specific rune-terrain (blue vs red) effects on early jungle. The dataset already has `got_first_blood`, `first_dragon_me`, `first_tower_me`; try encoding their **temporal interactions** - e.g., got first blood AND first dragon both before minute 10.

### Run 27 - 2026-04-24 - minute-to-minute dominance consistency features (5 new)
- Hypothesis: existing aggregates summarize the full 0..15 window but don't distinguish "steady lead" from "recent surge." Add phase-local dominance: `max_consec_ahead_min`, `max_consec_behind_min`, `early_dominance_rate` (0..5), `late_dominance_rate` (10..15), `final3_positive_count` (13..15). These encode minute-to-minute persistence that neither the existing aggregates nor polynomials capture.
- Config: append 5 row-computed features in `eng()` using per-minute `m_0..m_15_gold_diff` columns and `rle`-based run-length detection. Everything else unchanged from Run 21.
- Result: AUC **0.81695** | ACC **0.74926** | Baseline ACC **0.72640** | N_FEATURES **473** | DUR **29.05s**
- Decision: **DISCARD**. AUC regressed `-0.00076` vs Run 21. ACC unchanged. The dominance-persistence features are highly correlated with `gold_adv_time_share`, `gold_lead_changes`, and `gold_diff_last3_mean`, which already capture most of the persistence signal. Adding 5 more correlated features forced ENET's L1 component to split weight more thinly across redundant columns.
- Signal for next run: adding features from the same conceptual family (dominance/persistence) is saturating. Genuinely new angle: **pressure / threat proximity features** - distance to game-ending thresholds at minute 15. E.g., `gold_lead_vs_deathball := pmax(0, f_15_gold_diff - 3000) / 1000` flags leads large enough to snowball; `tower_push_momentum := f_15_tower_diff - f_10_tower_diff` captures map-pressure acceleration. These describe **structural closeness to winning the map**, not gold magnitudes per se.
